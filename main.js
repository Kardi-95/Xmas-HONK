...
class PlayScene extends Phaser.Scene {
  ...
  spawnPair() {
    ...
    [top, bot].forEach(o => {
      ...
      if (isFire) this.attachFlameTo(o);
    });
  ...
  attachFlameTo(chimneySprite) {
    const flame = this.add.image(chimneySprite.x, chimneySprite.y + 310, "flame").setDepth(6);
    flame.setAlpha(0.0);
    flame.setScale(0.9);
    chimneySprite.setData("flame", flame);

    const pulseOnce = () => {
      if (!chimneySprite.active || this.dead) { flame.destroy(); return; }
      ...
      if (on) {
        ...
        this.tweens.add({
          ...
          yoyo: true,
          repeat: 2,
          onComplete: () => this.time.delayedCall(delay, pulseOnce)
        });
      } else {
        flame.setAlpha(0.0);
  ...
  update() {
    ...
    // keep flames attached
    this.obstacles.children.each((o) => {
      if (!o.active) return;
      const flame = o.getData("flame");
      if (flame && flame.active) {
        flame.x = o.x;
        flame.y = o.y + 310;
        if (o.x < -140) flame.destroy();
      }
