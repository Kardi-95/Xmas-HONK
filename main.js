// main.js (FULL, CLEAN, WORKING)

// --- Load chapter JSON (no top-level await) ---
const CONFIG_URL = new URL("./chapter_xmas_01.json", import.meta.url).href;
...
function pickVariantId(variants) {
  if (!variants || !variants.length) return "chimney_normal";
...
// --- Bootstrap Phaser AFTER chapter loaded ---
function startGameWithChapter(chapter) {
  class PlayScene extends Phaser.Scene {
    constructor() {
      super("play");

      // A1: cookies only score
      this.cookies = 0;
    ...
    create() {
      ...
      this.cookieIcon = this.add
        .image(W / 2 - 34, 30, "cookie")
        .setScale(0.75)
        .setDepth(10);
      ...
      this.subText = this.add
        .text(W / 2, 230, "Click / SPACE to Start", {
          fontFamily: "monospace",
          fontSize: "22px",
          color: "#ffffff",
        })
      ...
      // Apply base gravity setup but only when started
    ...
    getRamped() {
      ...
      const every = ramp.everyScore ?? 10;
      const steps = Math.floor((this.cookies || 0) / every);
    ...
    spawnPair() {
      ...
      if (!spawned) {
        const cookieChance =
          (chapter.collectibles ?? []).find((x) => x.id === "cookie_coin")?.spawn?.chance ?? 0.28;
    ...
    attachFireGatePair(xStart, centerY, gapHeight, velX) {
      ...
      flame.setAlpha(0.0);
      flame.setScale(1.4, Math.max(1.0, gapHeight / 38));
      ...
      // Keep references for cleanup/update
      ...
      const burstOnce = () => {
        ...
        const offDelay = 450 + Math.random() * 900;
        const onDuration = 180 + Math.random() * 140;
        ...
        // Safety off (in case timings drift)
    ...
    hitFireGate() {
      // Only kills when enabled; body.enable is toggled ON/OFF
      if (this.time.now < this.invincibleUntil) return;
      this.die();
    }
    ...
    update() {
      ...
      this.cookiesGroup.children.each((c) => {
        if (c.active && c.x < -160) c.destroy();
      });

      this.powerups.children.each((p) => {
        if (p.active && p.x < -160) p.destroy();
      });

      // cleanup + keep fire visuals aligned
      this.fireGates.children.each((g) => {
        if (!g.active) return;

        const flame = g.getData("flameSprite");
        const glow = g.getData("glowSprite");
...
loadChapter()
  .then((chapter) => {
    startGameWithChapter(chapter);
  })
  .catch((e) => {
    const msg = (e && e.message) ? e.message : String(e);
    console.error(e);
    document.body.innerHTML = `<pre style="color:#fff;font:14px/1.4 monospace;padding:16px;">HO-HO-HONK failed to load 😬\n\n${msg}\n\nQuick checks:\n- Does chapter_xmas_01.json exist in repo root?\n- Are index.html / main.js in repo root?\n- Hard refresh (Cmd+Shift+R)\n</pre>`;
  });
