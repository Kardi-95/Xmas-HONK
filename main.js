...
class PlayScene extends Phaser.Scene {
  ...
  attachFireGatePair(xStart, centerY, gapHeight, velX) {
    ...
    const zone = this.add.zone(xStart, centerY, 76, gapHeight);
    ...
    const burst = () => {
      ...
      const offDelay = 520 + Math.random() * 900;   // time between bursts
      const onDuration = 160 + Math.random() * 180; // burst length
  ...
  showGameOver() {
    ...
    const cookie = this.add.image(W/2 - 30, H/2 - 18, KEYS.cookie).setScale(0.9).setDepth(52);
    const score = this.add.text(W/2 + 18, H/2 - 34, String(this.cookies), {
      fontFamily: "monospace",
      fontSize: "44px",
      color: "#ffffff",
    ...
    const ui = this.add.container(0, 0, [overlay, panel, title, cookie, score, hint]).setDepth(50);
    ui.setAlpha(0);
    ui.setScale(0.75);
